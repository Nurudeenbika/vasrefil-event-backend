...
exports.getRevenueStats = exports.getDashboardStats = void 0;
...
const getDashboardStats = async (req, res) => {
    try {
        ...
        const popularEvents = await Booking_1.default.aggregate([
            { $match: { status: "confirmed" } },
            {
                $group: {
                    _id: "$event",
        ...
        const categoryStats = await Event_1.default.aggregate([
            {
                $lookup: {
                    from: "bookings",
                    localField: "_id",
        ...
        // Get recent bookings
        const recentBookings = await Booking_1.default.find({ status: "confirmed" })
            .sort({ createdAt: -1 })
            .limit(10)
